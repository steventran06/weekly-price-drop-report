...
async function main(): Promise<void> {
  ...
  console.log("Community images: direct official-page metadata fetch, no AI call");
  ...
  if (tracked.addedBuilders.length > 0) {
    console.log(
      ...
      tracked.addedBuilders.map((builder) => builder.name).join(", "),
    );
  }

  console.log(`Builders researched this run: ${previous.builders.length}`);

  const verifiedDate = getPacificDate();
  const concurrency = readConcurrency();

  console.log(`Builder research concurrency: ${concurrency}`);

  const work = previous.builders.map((builder) => async () => {
    const existing = previous.communities.filter(
      (community) => community.builderId === builder.id,
    );

    try {
      const research = await researchBuilder(
        builder,
        existing,
        verifiedDate,
      );

      const merged = mergeBuilderResearch(
        builder,
        existing,
        research,
        verifiedDate,
      ...
      console.log(
        ...
        `incentives ${merged.audit.builderIncentiveCount + merged.audit.communityIncentiveCount})`,
      );

      return merged;
    } catch (error) {
      const preserved = preserveBuilderAfterResearchFailure(
        builder,
        existing,
        error,
      );
      ...
      console.warn(`  ${preserved.audit.error}`);
  ...
  const builders = results.map((item) => item.builder);
  const audits = results.map((item) => item.audit);
  const mergedCommunities = results
    .flatMap((item) => item.communities)
    .sort(
      (a, b) =>
        a.city.localeCompare(b.city) ||
        a.builderId.localeCompare(b.builderId) ||
        a.name.localeCompare(b.name),
    );

  const imageResult = await enrichCommunityImages(
    builders,
    mergedCommunities,
  );
  const communities = imageResult.communities;

  const next: NewConstructionData = {
    ...previous,
    schemaVersion: Math.max(
      Number(previous.schemaVersion || 1),
      3,
    ),
    ...
    feedMeta: {
      ...(previous.feedMeta || {}),
      source: "automated-official-builder-research-v5",
      researchedAt: new Date().toISOString(),
      researchDate: verifiedDate,
      researchMethod:
      ...
      trackedBuilderRegistryVersion: 1,
      trackedBuilderCount: previous.builders.length,
      failedBuilderCount: audits.filter(
        (audit) => audit.failed,
      ).length,
      uncertainCommunityCount: audits.reduce(
        (sum, audit) => sum + audit.uncertain,
        0,
      ),
      builderIncentiveCount: audits.reduce(
        (sum, audit) => sum + audit.builderIncentiveCount,
        0,
      ),
      communityIncentiveCount: audits.reduce(
        (sum, audit) => sum + audit.communityIncentiveCount,
        0,
      ),
      communityImageCount: imageResult.stats.found,
      communityLogoFallbackCount: imageResult.stats.fallback,
  ...
  const url = await publishWebsiteNewConstruction(
    next,
    context,
  );
...
function printAuditSummary(
  ...
  const totals = audits.reduce(
    (acc, audit) => ({
      kept: acc.kept + audit.kept,
      updated: acc.updated + audit.updated,
      removed: acc.removed + audit.removed,
      uncertain: acc.uncertain + audit.uncertain,
      added: acc.added + audit.added,
      failed: acc.failed + (audit.failed ? 1 : 0),
      builderIncentives:
        acc.builderIncentives + audit.builderIncentiveCount,
      communityIncentives:
        acc.communityIncentives + audit.communityIncentiveCount,
    }),
    {
      kept: 0,
      updated: 0,
  ...
  console.log("----------------");
  console.log(`Verified unchanged: ${totals.kept}`);
  console.log(`Updated with current details: ${totals.updated}`);
  console.log(`New: ${totals.added}`);
  console.log(`Removed: ${totals.removed}`);
  console.log(`Not freshly verified: ${totals.uncertain}`);
  console.log(`Builder calls failed/skipped: ${totals.failed}`);
  console.log(
...
function readConcurrency(): number {
  const value = Number(
    process.env.NEW_CONSTRUCTION_RESEARCH_CONCURRENCY || 2,
  );
...
main().catch((error) => {
  console.error("");
  console.error("New-construction research failed before builder processing completed.");
  console.error(error);
  process.exitCode = 1;
});
